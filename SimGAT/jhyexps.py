...
from sklearn.metrics import normalized_mutual_info_score, adjusted_rand_score, fowlkes_mallows_score, v_measure_score
...
def my_KNN(x, y, k=5, split_list=[0.2, 0.4, 0.6, 0.8], time=10, show_train=True, shuffle=True):
    ...
    return max(macro_list), max(micro_list)
...
def my_Kmeans(x, y, k=4, time=10, return_NMI=True):
    ...
    if time:
        ...
        s2 = max(ARI_list)
        fmi = max(FMI_list)
        #print('NMI (10 avg): {:.4f} , ARI (10avg): {:.4f}, FMI (10 avg): {:.4f}'.format(score, s2, fmi))
