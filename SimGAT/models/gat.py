...
class HeteGAT_multi(BaseGAttN):
    def inference(inputs_list, nb_classes, nb_nodes, training, attn_drop, ffd_drop,
                  bias_mat_list,sim_mat_list, hid_units, n_heads, activation=tf.nn.elu, residual=False,
                  mp_att_size=128):
        embed_list = []
        nodeattall = []
 
        for inputs, bias_mat, sim_mat in zip(inputs_list, bias_mat_list, sim_mat_list):
            attns = []
            jhy_embeds = []
            nodeattmetapath = []
			
            for _ in range(n_heads[0]):
                temp_att, nodeatt = layers.attn_head(inputs, bias_mat=bias_mat,nodesim=sim_mat,
                                              out_sz=hid_units[0], activation=activation,
                                              in_drop=ffd_drop, coef_drop=attn_drop, residual=False)
                attns.append(temp_att)
                nodeattmetapath.append(nodeatt)
                #attns.append(layers.attn_head(inputs, bias_mat=bias_mat,
                                              #out_sz=hid_units[0], activation=activation,
                                              #in_drop=ffd_drop, coef_drop=attn_drop, residual=False))
            h_1 = tf.concat(attns, axis=-1)
            ...
            nodeattall.append(nodeattmetapath)
        ...
        return logits, final_embed, att_val,nodeattall
