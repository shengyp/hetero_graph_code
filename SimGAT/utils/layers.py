...
def attn_head(seq, out_sz, bias_mat, nodesim,activation, in_drop=0.0, coef_drop=0.0, residual=False,
              return_coef=True):
    """[summary]
    ...
    with tf.name_scope('my_attn'):
        ...
        #coefs = nodesim
        if coef_drop != 0.0:
            nodesim = tf.nn.dropout(nodesim, 1.0 - coef_drop)
        if in_drop != 0.0:
            seq_fts = tf.nn.dropout(seq_fts, 1.0 - in_drop)


        vals = tf.matmul(nodesim, seq_fts)
